...
async def load_image_paths(base_path):
    '''
    Charge les chemins des images depuis un répertoire'''
    image_paths = {}
...
async def generate_images(label, fromages, prompts, images, ip_model, cfg, negative_prompt,i):
    ...
    for j in range(nb_prompts):
        ...
        images_generes = ip_model.generate(
...
async def generate(cfg):
    ...
    # Charger les labels, les fonds, les cadrages et les contextes
    labels = await load_labels("/users/eleves-b/2022/edouard.rabasse/Documents/INF473V-challenge-1/list_of_cheese2.txt")
    ...
    for label in labels:
        for i in range(num_images_per_label):
            ...
            # Construire un prompt aléatoire
            ...
            # prompt = f"best quality,high quality, on a totally white background"
            
            # Selectionner 3 images aléatoires pour chaque (prompt,label)
            selected_image_paths = random.sample(image_paths[label], 3)
